...
import (
	...
	testingv1beta3 "knative.dev/eventing/pkg/reconciler/testing/v1beta3"
)
...
func TestBuildEventMesh(t *testing.T) {
	tests := []struct {
		...
		eventTypes   []*eventingv1beta3.EventType
	...
	}{
		{
			...
			eventTypes: []*eventingv1beta3.EventType{
				testingv1beta3.NewEventType("test-eventtype", "test-ns",
					testingv1beta3.WithEventTypeType("test-eventtype-type"),
					testingv1beta3.WithEventTypeReference(brokerReference("test-broker", "test-ns")),
					// following fields are not used in any logic and simply returned
					testingv1beta3.WithEventTypeDescription("test-eventtype-description"),
					WithEventTypeUID("test-eventtype-uid"),
					WithEventTypeSchema(&apis.URL{
						Scheme: "http",
						Host:   "test-eventtype-schema",
					}),
					testingv1beta3.WithEventTypeLabels(map[string]string{"test-eventtype-label": "foo"}),
	...
	for _, tt := range tests {
		...
		fakeEventingClient := fakeclientset.NewSimpleClientset(v1beta2objects...)

		t.Run(tt.name, func(t *testing.T) {
			got, err := BuildEventMesh(ctx, fakeEventingClient, fakeDynamicClient, logger)
			if (err != nil) != tt.error {
...
func WithEventTypeUID(uid string) testingv1beta3.EventTypeOption {
	return func(a *eventingv1beta3.EventType) {
		a.UID = types.UID(uid)
	}
}

func WithEventTypeSchema(url *apis.URL) testingv1beta3.EventTypeOption {
	return func(a *eventingv1beta3.EventType) {
		a.Spec.Attributes = append(a.Spec.Attributes, eventingv1beta3.EventAttributeDefinition{
			Name:  "schema",
			Value: url.String(),
		})
	}
...
func WithEventTypeAnnotations(annotations map[string]string) testingv1beta3.EventTypeOption {
	return func(a *eventingv1beta3.EventType) {
